...
> **Pitfalls:** `tasks` does **not** have `agent_id` → use `agent_assigne_id`. `task_comments.agent_id` (not `auteur_agent_id`).
...
INSERT INTO task_comments (task_id, agent_id, contenu) VALUES (:task_id, :agent_id, '<content>');

-- Lock a file (BEFORE modification)
INSERT OR REPLACE INTO locks (fichier, agent_id, session_id) VALUES ('<file>', :agent_id, :session_id);
