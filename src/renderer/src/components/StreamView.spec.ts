...
describe('StreamView', () => {
  ...
  // T1764: AskUserQuestion / pendingQuestion tests
  it('shows pending question banner from synthetic ask_user event (Source 2, T1764)', async () => {
    const askUserEvent: StreamEvent = { type: 'ask_user', text: 'What color do you prefer?' }
    const { wrapper } = await mountStream([askUserEvent])
    await nextTick()
    const banner = wrapper.find('[data-testid="pending-question-banner"]')
    expect(banner.exists()).toBe(true)
    expect(banner.text()).toContain('What color do you prefer?')
  })

  it('shows pending question banner from AskUserQuestion tool_use block (Source 1, T1764)', async () => {
    const assistantEvent: StreamEvent = {
      type: 'assistant',
      message: {
        role: 'assistant',
        content: [{ type: 'tool_use', name: 'AskUserQuestion', input: { question: 'Which option?' } }],
      },
    }
    const { wrapper } = await mountStream([assistantEvent])
    await nextTick()
    const banner = wrapper.find('[data-testid="pending-question-banner"]')
    expect(banner.exists()).toBe(true)
    expect(banner.text()).toContain('Which option?')
  })

  it('falls back to Source 2 (ask_user) when AskUserQuestion input.question is missing (T1764)', async () => {
    // Source 1 finds the tool_use block but input.question is missing → must NOT return null
    // Source 2 should still find the synthetic ask_user event
    const assistantEvent: StreamEvent = {
      type: 'assistant',
      message: {
        role: 'assistant',
    ...
    const askUserEvent: StreamEvent = { type: 'ask_user', text: 'Fallback question text?' }
    const { wrapper } = await mountStream([assistantEvent, askUserEvent])
    await nextTick()
    const banner = wrapper.find('[data-testid="pending-question-banner"]')
    expect(banner.exists()).toBe(true)
    expect(banner.text()).toContain('Fallback question text?')
  })

  it('hides pending question banner after user reply (T1764)', async () => {
    const assistantEvent: StreamEvent = {
      type: 'assistant',
      message: {
        role: 'assistant',
    ...
    const userReply: StreamEvent = {
      type: 'user',
      message: { role: 'user', content: [{ type: 'text', text: 'Option A' }] },
    }
    const { wrapper } = await mountStream([assistantEvent, userReply])
    await nextTick()
    const banner = wrapper.find('[data-testid="pending-question-banner"]')
    expect(banner.exists()).toBe(false)
  })

  it('AskUserQuestion block rendered inside assistant event (T1764)', async () => {
    const assistantEvent: StreamEvent = {
      type: 'assistant',
      message: {
        role: 'assistant',
        content: [{ type: 'tool_use', name: 'AskUserQuestion', input: { question: 'What do you want?' } }],
      },
    }
    const { wrapper } = await mountStream([assistantEvent])
    await nextTick()
...
describe('StreamView auto-close on process exit (T1373)', () => {
  ...
  it('does NOT close tab on exit when agent.type=task-creator (T1373)', async () => {
    const { tabsStore } = await mountWithCli('aider', { agentType: 'task-creator' })
    const closeTabSpy = vi.spyOn(tabsStore, 'closeTab')
    vi.useFakeTimers()
